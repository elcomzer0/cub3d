...
/*   Updated: 2024/09/09 19:46:39 by miturk           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*trim(char *s, char *tmp)
{
	if (ft_strncmp(s, "NO ", 3) == 0)
		...
		return (free(s), ft_putstr_fd("Error: Invalid texture\n", 2), NULL);
	if (tmp == NULL)
		return (ft_putstr_fd("Error: Malloc failed\n", 2), NULL);
	return (free(s), tmp);
}
...
int	call_color(t_file *data, char *s)
{
	if (ft_strncmp(s, "F ", 2) == 0 && data->text->f == NULL)
	{
		...
		if (data->text->f == NULL)
			return (free(s), ft_putstr_fd("Error: Malloc failed1\n", 2), false);
	}
	else if (ft_strncmp(s, "C ", 2) == 0 && data->text->c == NULL)
	{
		...
		return (ft_putstr_fd("Error: Invalid texture: color\n", 2),
			false);
	return (true);
}

...
/*   Updated: 2024/09/13 15:39:35 by miturk           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	the_start(t_data *data, char **av)
{
	...
	if (text == NULL)
		return (ft_putstr_fd("Error:\nMalloc failed\n", 2),
			free(data->file), free(data->player), free(data), 1);
	data->file->text = text;
	if (data->file->text == NULL)
		return (ft_putstr_fd("Error:\nMalloc failed\n", 2),
			free(text), free(data->file), free(data->player), free(data), 1);
	if (work_split(data, data->file, av) == false)
		return (free(data->player), free(text),
			free(data->file), free(data), 1);
	data->z_values = NULL;
	if (map_conversion(data, 0) == false)
		return (free(text), free(data->file),
			free(data->player), free(data), 1);
